...
(define (translate-llvm mini*)
  (match mini*
    [(Mini types decs funs)
     (define locs (make-immutable-hash (map (lambda (dec) (cons (car dec) (@ (car dec)))) decs)))
     (LLVM (map translate-struct types)
           (map translate-global decs)
           (map (lambda (f) (translate-fun f locs)) funs))]))
...
(define (translate-fun f locs)
  (match f
    [(Fun id params ret-type decs body)
     (let* ([all-locs (hash-union locs (get-locs params decs) #:combine (lambda (a b) b))]
            [blocks (map (lambda (b) (translate-block b all-locs)) body)])
...
(define (translate-block b locs)
  (BlockLL (Block*-id b)
           (append-map (lambda (s) (translate-stmt s locs)) (Block*-stmts b))))
...
(define (translate-stmt s locs)
  (define stmts (box '()))
  (define add-stmt! (lambda (s) (set-box! stmts (cons s (unbox stmts)))))
  (match s
...
(define (translate-arg arg locs add-stmt!)
  (match arg
    ...
    [(Prim op (list op1 op2))
     (let ([i (make-i)])
       (add-stmt! (BinaryLL i (hash-ref ops op) 'bad (translate-arg op1 locs add-stmt!) (translate-arg op2 locs add-stmt!)))
       i)]
    [o o]))
...
(define (get-locs params decs)
  (make-immutable-hash (append
                        (map (lambda (d) (cons (car d) (% (car d)))) decs)
                        (map (lambda (d) (cons (car d) (% (param-loc (car d))))) params))))
   ...
   (map (λ (dec) (AllocLL (% (car dec)) (translate-type (cdr dec))))
        (append
         (map (λ (dec) (cons (format "_P_~a" (car dec)) (cdr dec)))
              params)
         decs))
   (map (lambda (dec) (StoreLL (translate-type (cdr dec))
                               (% (car dec))
