...
(define (control-flow-fun fun)
  (let* ([cfg (box '())]
         ...
         [decs (Fun-decs fun)])
    (add-block! end-id (list (Return (if void-ret? (void) '_ret_val_))))
    (add-block! start-id (stmt-cfg (Fun-body fun) add-block! (Goto* end-id) end-id))
    (struct-copy Fun fun [body (unbox cfg)]
                 [decs (if void-ret? decs (cons (cons '_ret_val_ (Fun-ret-type fun)) decs))])))
...
(define (stmt-cfg body add-block! next ret-id)
  (match body
    ...
    [(cons (Return exp) _) (list (Assign '_ret_val_ exp) (Goto* ret-id))]
