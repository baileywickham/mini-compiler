  ...
  (match body
    ...
    [(cons (While guard while-body) after)
     (with-labels (while-id after-id)
       (translate-body while-body (add-block! while-id #f) add-block! add-stmt!
                       (GotoCond* guard while-id after-id) ret-id)
       (translate-body after (add-block! after-id #t) add-block! add-stmt! next ret-id)
       (add-stmt! current-block (GotoCond* guard while-id after-id)))]
    [(cons (Return (? void?)) _)
     (add-stmt! current-block (Goto* ret-id))]
    [(cons (Return exp) _)
     (add-stmt! current-block (Assign return-var exp))
     (add-stmt! current-block (Goto* ret-id))]
    [(cons stmt rest)
     (translate-stmt stmt current-block add-stmt!)
